...
/* ===== Spaltennamen (GENAU wie im Sheet) ===== */
...
/* ===== Funktions-Rollen (IDs von dir eintragen) ===== */
...
/* ===== Dienstzuteilung → Rollen (IDs von dir eintragen) =====
   Beispiel: "Feuerwehr Wiener Neustadt": "ROLE_ID_FFWN"
*/
...
/* ===== Abkürzungs-Aliase für Dienstgrade (optional) ===== */
const RANK_ALIASES = {
...
/* ===== Normalisierung Dienstgrad ===== */
function normalizeGrade(raw) {
  const s = String(raw || '').trim();
  if (!s) return '';
  const key = s.toUpperCase().replace(/\s+/g, ' ');
  return RANK_ALIASES[key] || s;
}

/* ===== Einzelne Zielrollen bestimmen ===== */
function rankRoleIdFor(rec) {
  const norm = normalizeGrade(rec[COL_GRADE]);
  return RANK_ROLE_MAP[norm] || null;
}
...
/* ===== Gesamte Zielrollen bestimmen ===== */
function desiredRoleIdsFor(rec) {
  const ids = [];
  // Kurs-/Lehrgangsrollen (alle "JA"-Spalten)
...
async function setNickWithDiagnostics(guild, member, desiredNick, changes) {
  ...
  if (!me.permissions.has(PermissionFlagsBits.ManageNicknames))
    return changes.push('nick: Bot hat keine Permission ManageNicknames');
  if (member.id === guild.ownerId)
    return changes.push('nick: Server-Owner kann nicht umbenannt werden');
  if (member.roles.highest.comparePositionTo(me.roles.highest) >= 0)
    return changes.push('nick: Rollen-Hierarchie blockiert');

  const nick32 = capNick(desiredNick);
  await member.setNickname(nick32).catch(e => changes.push(`nick: ${e.message}`));
}
...
/* v14 deprecation: "ready" → "clientReady" */
...
client.on('interactionCreate', async (i) => {
  ...
  if (i.commandName === 'sync_member') {
    ...
    if (!rec) return i.editReply(`Kein Datensatz für **${userId}** gefunden.`);
    const res = await syncMember(guild, rec);
    return i.editReply(res.ok
      ? `OK. Nick: **${activeNicknameOf(rec)}** | added: ${res.details.added.length} | removed: ${res.details.removed.length}`
      : `Info: ${res.reason || res.changes?.join(', ') || 'siehe Logs'}`);
  }
});
